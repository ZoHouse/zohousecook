...
|---|---|---|
| Left panel (absent today) | — | `<aside>` at `absolute left-8 top-32 z-[6]`, fixed `w-[320px]`, renders `ActiveQuestCard` on top and `GhostVisitors` beneath it with `space-y-6`. No mount guard — both slots are always truthy; empty-state visual behavior is handled via Tailwind zero-height collapse (see "Empty-state handling"). |
