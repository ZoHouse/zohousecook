-- Cafe — fix place_cafe_order food-credit unit mismatch + uuid casts
--
-- Two bugs in the customer-flow RPC, both pre-existing (since 20260404)
-- and surfaced only when /cafezomad/[tableId].tsx was finally exercised
-- in production. Staff orders via CreateOrderDialog do direct inserts,
-- so neither bug ever fired through them.
--
-- Bug 1 (food credit unit mismatch): place_cafe_order compared and
-- debited food_credit_wallets.balance (stored in RUPEES) against
-- p_food_credit_paise (PAISE) directly, off by 100x. A wallet with
-- ₹1602 read as 1602 paise, every customer order with food credits
-- failed with "insufficient: have 1602, need 18500" for a ₹185 order.
--
-- Bug 2 (text→uuid): cafe_orders.table_id and cafe_orders.zo_user_id
-- are uuid columns, but the RPC declares p_table_id and p_zo_user_id
-- as text and inserts them unchanged. Postgres has no assignment-cast
-- from text to uuid, so INSERT fails with "column is of type uuid but
-- expression is of type text". Fixed by explicit ::uuid casts on both,
-- with NULLIF on p_zo_user_id so empty strings become NULL instead of
-- triggering "invalid input syntax for type uuid: ''".
--
-- Schema convention (verified from existing ledger):
--   cafe_orders.food_credit_applied_paise → PAISE (column name correct)
--   food_credit_wallets.balance           → RUPEES
--   food_credit_transactions.amount       → RUPEES
--   cafe_orders.table_id                  → uuid
--   cafe_orders.zo_user_id                → uuid
--
-- Fix 1: introduce v_food_credit_rupees = floor(p_food_credit_paise / 100)
-- and use it for wallet sufficiency check, wallet debit, and ledger insert.
-- cafe_orders.food_credit_applied_paise stays in paise. Return JSON keeps
-- food_credit_applied in paise for frontend compat.
--
-- Fix 2: in INSERT INTO cafe_orders, cast p_table_id::uuid and
-- NULLIF(p_zo_user_id, '')::uuid.
--
-- Same 8-arg signature, so CREATE OR REPLACE works in place. No DROP.
...
BEGIN
  ...
  IF p_food_credit_paise > 0 THEN
    ...
    -- Wallet balance is RUPEES; v_food_credit_rupees converts the PAISE input.
  ...
  -- Debit wallet in RUPEES (matches schema convention).
