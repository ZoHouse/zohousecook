-- Cafe — accepting_orders flag on cafe_properties + RPC enforcement
--
-- Akhilesh: "kitchen board also needs a pause orders function". When the
-- chef is slammed, ingredients run out, or service hours end, staff need
-- a one-click way to stop new customer orders without taking down the
-- whole property.
--
-- Schema: cafe_properties.accepting_orders (bool, default true). True = open
-- for orders. False = closed; place_cafe_order rejects + customer cafezomad
-- page shows "We're closed right now" instead of menu.
--
-- RPC: place_cafe_order checks the flag at the very top, raises if false.
-- Existing callers don't need to change — they just see a clean error
-- bubble up.

ALTER TABLE cafe_properties
  ADD COLUMN IF NOT EXISTS accepting_orders boolean NOT NULL DEFAULT true;

-- Re-create place_cafe_order with the gate. Same 9-arg signature.
...
-- Allow anon to update accepting_orders (staff toggle uses the anon key
-- like every other PMS write). Mirrors anon write policies on cafe_orders.
DROP POLICY IF EXISTS "anon_update_properties" ON cafe_properties;
CREATE POLICY "anon_update_properties" ON cafe_properties
