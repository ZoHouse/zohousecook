...
const JsonInput: React.FC<JsonInputProp> = ({
  ...
  return (
    <div className="w-full bg-zui-light rounded-xl px-6 py-4 relative">
      <label
        className={cn(
          "text-base transition-all ease-in-out duration-100 text-zui-silver"
        )}
        htmlFor={name}
      >
        {label} {required && <span className="text-zui-silver">*</span>}
      </label>
      <div className="relative mt-4 flex border border-zui-lightest">
        <div
          ref={lineNumbersRef}
        ...
        <textarea
          ...
          onScroll={syncScroll}
          className="bg-zui-light w-full focus:outline-none text-sm resize-none p-2 leading-6 font-mono overflow-auto whitespace-nowrap"
