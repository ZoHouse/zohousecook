...
import { ViewEventsModal } from "../../../components/helpers/san-francisco";
import { HeroSection } from "../../../components/helpers/cryptoEvents";
...
> = ({ metaData }) => {
  ...
  >(
    "BOOKINGS_EXPERIENCE_OPERATORS",
    {
      ...
      enabled: isValidString(process.env.CRYPTO_EVENT_PID),
      onSuccess(data) {
    ...
    `${process.env.CRYPTO_EVENT_PID}/inventory`,
    `start_at__gte=${moment().format("yyyy-MM-DD")}`
  );

  const { data: operatorData } = useQueryApi<BookingOperatorResponse>(
    "BOOKINGS_EXPERIENCE_OPERATORS",
    {
      select: (data) => {
        const allEvents: BookingExperienceResponse[] = data.data.results;
  ...
  const initialCoordinates: { lng: number; lat: number } = useMemo(() => {
    // Default coordinates if no data available
    const defaultCoordinates = {
      lng: operatorData?.estate?.location?.coordinates[0] ?? 0,
      lat: operatorData?.estate?.location?.coordinates[1] ?? 0,
    };
    ...
    const validEvents = eventsData.filter(
      (event) => event.longitude && event.latitude
    );
    ...
    const sortedLng = validEvents
      .map((event) => +event.longitude)
      .sort((a, b) => a - b);
    const sortedLat = validEvents
      .map((event) => +event.latitude)
      .sort((a, b) => a - b);

    const midIndex = Math.floor(sortedLng.length / 2);

    return {
      lng: sortedLng[midIndex],
      lat: sortedLat[midIndex],
    };
  }, [eventsData, operatorData]);
