...
// Inline replacements for dashboard-specific components
function ZoSpinner({ size = 24 }: { size?: number }) {
  return <div className="animate-spin rounded-full border-2 border-white/20 border-t-white" style={{ width: size, height: size }} />;
}
function GlassCard({ children, className = "" }: { children: React.ReactNode; className?: string }) {
  return <div className={`bg-white/5 backdrop-blur-md rounded-2xl border border-white/10 ${className}`}>{children}</div>;
}
...
function ProfileHeader() {
  ...
  return (
    <div className="flex items-center gap-4 mb-2">
      <div className="w-14 h-14 rounded-full overflow-hidden border-2 border-dash-border flex-shrink-0">
        {avatar && !imgError ? (
          ...
          <div className="w-full h-full bg-gradient-to-br from-dash-accent/30 to-dash-accent/10 flex items-center justify-center">
            <span className="text-lg font-bold text-dash-text">{name.charAt(0).toUpperCase()}</span>
          </div>
        )}
      </div>
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-2">
          <h3 className="text-lg font-bold text-dash-text truncate">{name}</h3>
          {isFounder && (
        ...
        <p className="text-xs text-dash-text-50 capitalize">{membership || "Citizen"} of Zo World</p>
      </div>
    </div>
  );
}
...
export function SettingsDrawer({ isOpen, onClose }: SettingsDrawerProps) {
  return (
    <>
      ...
      {/* Drawer */}
      <div className={`fixed right-0 top-0 h-full w-[480px] max-w-full bg-[#111] z-50 overflow-y-auto transition-transform duration-300 ${isOpen ? "translate-x-0" : "translate-x-full"}`}>
        <div className="p-6">
          {/* Header */}
