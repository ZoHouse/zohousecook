...
export function CitizenCard({ displayName, avatarUrl, onUpsell }: CitizenCardProps) {
  return (
    ...
    >
      <BorderBeam radius={CARD_RADIUS} duration={6} borderWidth={1.5} colorFrom="#A7D921" colorTo="#FF2F8E" trailDegrees={90} />

      {/* Avatar portrait */}
      <div style={{ borderRadius: 12, overflow: 'hidden', marginBottom: 10, width: 176, height: 176, position: 'relative', zIndex: 2 }}>
      ...
      <div
        ...
        style={{ fontSize: 24, fontWeight: 700, color: '#FFFFFF', lineHeight: '1.2em', marginBottom: 2, position: 'relative', zIndex: 2 }}
      >
        {displayName}
      </div>
      {/* Subtitle — Rubik 400, 12px, 55% opacity */}
      <div style={{ fontSize: 12, fontWeight: 400, color: 'rgba(255,255,255,0.55)', letterSpacing: '0.01em', position: 'relative', zIndex: 2 }}>
        Citizen of Zo World
      </div>
    </div>
  );
