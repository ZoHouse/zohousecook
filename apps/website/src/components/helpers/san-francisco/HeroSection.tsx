...
import ZoZoButton from "./ZoZoButton";
...
const zoHouseSfCoordinates: {
  lng: number;
  lat: number;
} = {
  lng: -122.40119628387531,
  lat: 37.781757303913,
};
...
const HeroSection: React.FC<HeroSectionProps> = ({
  ...
  const [sfTime, setSfTime] = useState<Moment>(moment().utcOffset(-7));
    ...
    if (mapRef.current) {
      const points =
        !event.longitude && !event.latitude
          ? {
              lat: zoHouseSfCoordinates.lat,
              lng: zoHouseSfCoordinates.lng,
            }
  ...
  const addUserMarker = useCallback(() => {
    if (!userLocation || !mapRef?.current || locationPermissionDenied) return;

    const userMarkerElement = document.createElement("img");
    userMarkerElement.src = `${process.env.MEDIA_BASE_URL}/gallery/media/images/9610a0d3-c1bf-4072-9e4b-7ab4f6afaec1_20250321094007.png`;
    userMarkerElement.className = "custom-mapbox-user-marker h-14 w-auto z-40";
  ...
  const plotMarkers = useCallback(
    (events: BookingExperienceResponse[]) => {
      ...
      new mapboxgl.Marker(zoHouseMarkerElement)
        .setLngLat(zoHouseSfCoordinates) // Set the position using the coordinates
  ...
  const handleJumpToZoHouseSFO = () => {
    if (!mapRef || typeof mapRef !== "object") return;
    if (userLocation && mapRef.current) {
      zoomToLocation(mapRef.current, zoHouseSfCoordinates, 15);
    }
  };
    ...
    if (navigator.geolocation) {
      navigator.geolocation.getCurrentPosition(
        ...
        (error) => {
          ...
          setUserLocation(zoHouseSfCoordinates);
          setLocationPermissionDenied(true);
        }
      );
    } else {
      console.error("Geolocation is not supported by this browser.");
      setUserLocation(zoHouseSfCoordinates); // Fallback location
      setLocationPermissionDenied(true);
    }
  }, []);
  ...
  useEffect(() => {
    const updateTimes = () => {
      ...
      const sfNow = moment().utcOffset(-7); // San Francisco is UTC-7 (PDT)
      setSfTime(sfNow);

      const hour = sfNow.hour();
  ...
  return (
    <>
      <section className="w-full h-[85vh] relative overflow-y-hidden">
        <div className="absolute top-6 md:left-6 px-6 md:px-0 z-20 w-full md:w-auto">
          ...
          <p className="md:hidden text-center text-sm font-semibold mt-2 px-2">
            Step into SF&apos;s dopest clubhouse for crypto natives <br /> and
            techno-optimists
          </p>
        ...
        <div className="hidden md:block absolute top-6 left-1/2 -translate-x-1/2 z-20">
          ...
          >
            ...
            <h5>{sfTime.format("HH:mm")}</h5>
            <span className="text-zui-silver">{sfTime.format(" [GMT]Z")}</span>
          </div>
