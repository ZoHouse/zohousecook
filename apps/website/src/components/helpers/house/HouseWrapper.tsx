...
export function HouseWrapper({ children }: { children: React.ReactNode }) {
  ...
  const handleTuneIn = () => {
    ...
    // re-render — this must run inside the click event so the browser sees a
    // valid user gesture. Any subsequent `new Audio().play()` will work,
    // including the DJ TTS clips that fire from async callbacks later.
    try {
      const unlock = new Audio(SILENT_MP3);
      unlock.volume = 0;
      void unlock.play().catch(() => {
        /* autoplay blocked — user will need to re-click the radio pill */
      });
    } catch {
