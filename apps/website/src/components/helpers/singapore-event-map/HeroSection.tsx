...
import { buildingLayer } from "./MapConfig";
...
const HeroSection: React.FC<HeroSectionProps> = () => {
  ...
  const [isSearchVisible, showSearch, hideSearch] = useVisibilityState();
  ...
  >(
    "BOOKINGS_EXPERIENCE_OPERATORS",
    {
      ...
      enabled: isValidString(process.env.SINGAPORE_OPERATOR_PID),
    },
    `${process.env.SINGAPORE_OPERATOR_PID}/inventory`
  );
  ...
  >("BOOKINGS_SEED", {
    refetchOnWindowFocus: false,
    select: (data) => {
      const options = data.data.inventory.subcategory
        .filter((category: string | null) => isValidString(category))
        .map((category: string) => ({
          label: category,
          icon: cultureMarkers[category.toLowerCase()],
          value: category.replace(/ /g, ""), // Use replace with global regex to remove spaces
        }));
  ...
  useEffect(() => {
    const updateUrl = debounce(() => {
      if (mapCenter) {
        ...
        router.push(
          `/events/token2049?lat=${lat}&long=${lng}&zoom=${zoom}`,
          undefined,
          { shallow: true }
        );
      }
      ...
      <section className="w-full h-[92vh] relative">
        <header className="top-12 absolute left-0 w-full z-10">
          <div className="flex items-center justify-center gap-3">
            ...
            <Link href={"/"}>
              <img
                className="h-8"
                src={`${process.env.MEDIA_BASE_URL}/gallery/media/images/8e1919fe-212e-47f0-a3cb-a3a63b92b56b_20240913134636.png`}
                alt="Token 2049 Singapore"
              />
            </Link>
          </div>

          <p className="text-sm font-medium text-center mt-2 mb-4">
            15 - 23 Sep • Official Community Partner
          </p>
        ...
        <div className="flex flex-col items-center absolute z-10 gap-2 bottom-72 right-2 md:right-0 md:left-0 md:justify-center md:bottom-8 md:flex-row">
          <button
            ...
            className="w-14 h-14 relative border border-zui-white/10 rounded-full flex justify-center items-center"
          >
            <img
              className="absolute w-2 aspect-square z-10 top-0 left-0"
            ...
            />
            <img
              className="absolute w-2 aspect-square z-10 right-0 bottom-0"
              src={`${process.env.MEDIA_BASE_URL}/gallery/media/images/2e7e3b93-1065-4f80-92b1-c2c818b09f64_20240911105307.gif`}
              alt="animated-stars"
            />
          </button>
          <button
            onClick={isSearchVisible ? hideSearch : showSearch}
            className="bg-zui-dark border border-zui-white/10 rounded-full p-4"
          >
            <Icon
              name={isSearchVisible ? "Cross" : "Search"}
              size={24}
              fill="#fff"
            />
          </button>
