...
export async function deductInventoryForOrder(
  ...
  if (oiErr || !orderItems?.length) return result
  ...
  if (rErr || !recipes?.length) {
    for (const oi of orderItems) {
      result.skipped.push({ menuItem: oi.name, reason: 'no recipe' })
    }
    return result
  }

  // 3. Build deduction map: ingredient_id → total quantity to deduct (in stock unit)
