...
interface BulkMediaUploadSidebarProps {
  isOpen: boolean;
  onClose: () => void;
...
const BulkMediaUploadSidebar: React.FC<BulkMediaUploadSidebarProps> = ({
  isOpen,
  onClose,
  onUploadSuccess,
}) => {
  ...
  useEffect(() => {
    if (fileList.length > 0 && mediaType) {
      form.setFieldsValue({ category: MEDIA_TYPES[mediaType].category });
    }
  }, [fileList, mediaType, form]);
    ...
    try {
      ...
      const uploadPromises = fileList.map(async (file) => {
        ...
        if (!file || !(file instanceof File)) {
          throw new Error(
            `Invalid file object for ${file.name}. Please try uploading the file again.`
          );
        }
        ...
        return uploadMedia(
          { data: mediaData },
          {
            onError(error) {
              message.error(
                `Failed to upload ${file.name}: ${processResponseError(error)}`
              );
            },
            onSuccess(data) {
  ...
  // Update the loadImage helper in applyWatermark to handle SVGs
  ...
  useEffect(() => {
    const generatePreview = async () => {
      ...
      try {
        const watermarkedFile = await applyWatermark(fileList[0]);
                    ...
                    <>
                      ...
                      >
                        <Switch
                          ...
                          disabled={fileList[0]?.type === "image/svg+xml"}
                          onChange={(checked) => {
...
export default BulkMediaUploadSidebar;
