...
export function DashboardHeader() {
  ...
  return (
    <header className="sticky top-0 z-50 flex items-center justify-between px-dash-xl py-dash-lg bg-dash-bg-secondary backdrop-blur-dash-md border-b border-dash-border">
      ...
      <div className="relative" ref={menuRef}>
        <button onClick={() => setMenuOpen(!menuOpen)} className="flex items-center gap-2">
          ...
          <span className="text-sm text-dash-text-80 hidden md:block">{nickname}</span>
        </button>
        {menuOpen && (
          <div className="absolute right-0 top-12 bg-dash-bg-solid border border-dash-border rounded-dash-md p-2 min-w-[160px] z-50">
            <button
